...
// ---- Backend hook stubs (overridden by backend-adapter.js when a Node server is running) ----
window.BackendHooks = {
  onLogin:      async () => null,
  onCaseSave:   async () => {},
  onEmailSend:  async () => {},
...
const Storage = {
  async save() {
    ...
    // Sync to backend when adapter is loaded (no-op by default)
    try {
      if (window.BackendHooks?.onCaseSave) await window.BackendHooks.onCaseSave(State.cases);
    } catch(e) {}
  },
  async load() {
    try {
