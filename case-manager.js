...
ხელმოწერილი შეთანხმებისა და ავანსის მიღების შემდეგ, დაუყოვნებლივ დავიწყებთ მუშაობას.
...
function _parseImportRows(text) {
  return text.trim().split('\n')
    ...
    .filter(cols => {
      ...
      if (/last.?name|^#$/i.test(cols[0]||'') || /last.?name/i.test(cols[1]||'')) return false;
      if (!/[A-Za-z]/.test(cols[1]||'') || !/[A-Za-z]/.test(cols[2]||'')) return false;
      return true;
    })
    .map(cols => {
      const fd = cols[7]||'', r = cols[9]||'';
      return newCase({
        lastName:          cols[1]||'',
        firstName:         cols[2]||'',
        visaType:          _visaFromCase(cols[3]||''),
        stage:             _stageFromData(fd, r),
        retainerAmount:    _xAmt(cols[4]),
        retainerPaid:      /pif/i.test(cols[6]||''),
        filingFeesAmount:  _xAmt((cols[5]||'').split(/[+\/]/)[0]),
        filingFeesPaid:    /pif/i.test(cols[6]||''),
        filingDate:        _xDate(fd),
        approvalDate:      _xApprDate(fd, r),
        uscisReceiptNumber: _xReceipt(r),
        priorityDate:      _xDate(cols[11]||''),
        notes: [
          cols[3]  ? `Case Type: ${cols[3]}` : '',
          cols[6]  ? `Payment: ${cols[6]}` : '',
          cols[10] ? `Status/Expiration: ${cols[10]}` : '',
          cols[9]  ? `Receipt/Notes: ${cols[9]}` : '',
          cols[12] ? `Officer: ${cols[12]}` : '',
          cols[8]  ? `RA Date: ${cols[8]}` : '',
        ].filter(Boolean).join('\n'),
      });
    });
}
